  ...
  function initializeTool() {
    ...
    function initLayer() {
      ...
      try {
        ...
        if (log) log("warn", "⚠️ tool.cierres: addSource falló (¿estilo no cargado?), reintentando en load/style.load:", err.message);
        App.map.once("load", () => initLayer());
        App.map.once("style.load", () => initLayer());
        return;
      }
      ...
      try {
        ...
        if (log) log("warn", "⚠️ tool.cierres: addLayer falló, reintentando en load/style.load:", err.message);
        App.map.once("load", () => initLayer());
        App.map.once("style.load", () => initLayer());
        return;
      }
...
`;
        ...
        function fallbackCopy(text, btn) {
          try {
            ...
            setTimeout(() => { btn.textContent = "📋 Copiar coordenadas"; }, (window.__FTTH_CONFIG__?.UI_COPY_FEEDBACK_MS) ?? 1500);
          } catch (err) {
            ...
            btnCopyCoords.addEventListener("click", function (e) {
              ...
              if (navigator.clipboard && navigator.clipboard.writeText) {
                navigator.clipboard.writeText(text).then(() => {
                  ...
                  setTimeout(() => { btnCopyCoords.textContent = "📋 Copiar coordenadas"; }, (window.__FTTH_CONFIG__?.UI_COPY_FEEDBACK_MS) ?? 1500);
