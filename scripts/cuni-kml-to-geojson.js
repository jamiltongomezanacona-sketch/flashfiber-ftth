#!/usr/bin/env node
/**
 * Convierte CUNI.kml a GeoJSON por molécula (CUxx): cables + cierres.
 * Uso: node scripts/cuni-kml-to-geojson.js [ruta/CUNI.kml]
 * Salida: geojson/FTTH/CUNI/CUxx/cables/ y cierres/
 */

const fs = require("fs");
const path = require("path");

const inputPath = process.argv[2] || path.join(process.env.USERPROFILE || "", "Desktop", "CUNI.kml");
const outputBase = path.join(__dirname, "..", "geojson", "FTTH", "CUNI");
...
/** Extrae moléculas CUxx con sus Placemarks (cables LineString + cierres Point) */
function parseCuniKml(kml) {
  const molecules = {};
  const folderRe = /<Folder>\s*(?:<visibility>[\s\S]*?<\/visibility>\s*)?<name>(CU\d+)<\/name>([\s\S]*?)<\/Folder>/gi;
  let m;
...
function main() {
  ...
  const molecules = parseCuniKml(kml);
  ...
  for (const molId of molIds) {
    ...
    for (const cable of data.cables) {
      ...
      const layerId = `FTTH_CUNI_${molId}_${safeName}`;
      cableLayers.push({
        type: "layer",
        id: layerId,
        label: cable.name,
      ...
      const geojson = {
        ...
        features: [{
          type: "Feature",
          properties: { name: cable.name, molecula: molId, central: "CUNI" },
    ...
    const cierreFeatures = data.cierres.map((c) => ({
      type: "Feature",
      properties: { name: c.name, molecula: molId, central: "CUNI", tipo: c.name.includes("E1") ? "E1" : "E0" },
  ...
  console.log("✅ GeoJSON CUNI escrito en", outputBase);
}
